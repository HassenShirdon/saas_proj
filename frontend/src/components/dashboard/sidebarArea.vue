<template>
  <div class="sidebar">
    <div class="sidebar-header">
      ...
      <div class="logo">
        <img
          ...
          class="logo-img"
        />
        <span class="logo-text">Marjaan</span>
      </div>
    </div>
    ...
    <div class="sidebar-footer">
      <!-- Optional: Footer content like settings, user info, or collapse button -->
...
export default {
  ...
  data() {
    return {
      navItems: [
        ...
        { name: 'inventory', path: '/analytics', icon: 'fas fa-chart-line' },
        { name: 'Finance', path: '/Finance', icon: 'fas fa-file-alt' },
  ...
  methods: {
    isActive(path) {
      // Basic active state check, adjust if your routing is more complex
...
.sidebar {
  ...
  height: 100%; /* Fill the height of its container in dashboardPage.vue */
  background-color: var(--dashboard-sidebar-bg, #2c3e50);
  color: var(--dashboard-sidebar-text-color, #ecf0f1);
  transition: width 0.3s ease-in-out; /* For responsive width changes */
}
...
.logo-text {
  font-size: 1.25rem;
  font-weight: 600;
  white-space: nowrap; /* Prevent text wrapping during width transition */
  overflow: hidden; /* Hide text if sidebar becomes too narrow */
}

.sidebar-nav {
  flex-grow: 1;
  overflow-y: auto; /* Allows scrolling for many nav items */
  padding-top: 15px;
}
...
.sidebar-nav li.active .nav-link {
  background-color: var(--dashboard-sidebar-hover-bg, #34495e);
...
.nav-icon {
  margin-right: 15px;
  font-size: 1.1rem;
  width: 20px; /* Ensure consistent spacing even if icon widths vary */
  text-align: center;
}
...
/* Styles for when sidebar is collapsed (e.g., icon-only mode) */
/* This would be triggered by a class on .sidebar or by media queries in dashboardPage.vue */
.sidebar.collapsed .logo-text,
.sidebar.collapsed .nav-text,
.sidebar.collapsed .sidebar-footer .nav-text {
  display: none; /* Or use opacity and width for smoother transitions */
}
...
/* Ensure Font Awesome is imported if you're using its classes like 'fas fa-tachometer-alt' */
