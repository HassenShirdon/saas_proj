<template>
  <nav class="top-navigation bg-white shadow-md p-3 rounded">
    <div class="nav-left">
      <div class="brand-logo">
        <!-- You can replace this with an <img> tag for your logo -->
        <span>Marjaan Solutions</span>
      </div>
      <!-- Optional: Hamburger menu for mobile sidebar toggle -->
      <!-- <button class="mobile-menu-toggle" @click="toggleSidebar">
        <i class="fas fa-bars"></i>
      </button> -->
    </div>

    <div class="nav-center">
      <!-- Optional: Search bar -->
      <div class="search-bar">
        <input type="text" placeholder="Search..." />
        <i class="fas fa-search"></i>
      </div>
    </div>

    <div class="nav-right">
      <div class="nav-item notifications">
        <!-- Replace with your notification icon and logic -->
        <i class="far fa-bell"></i>
        <span class="notification-badge">3</span>
      </div>
      <div class="nav-item user-profile">
        <img
          src="data:image/jpeg;base64,"
          alt="User Avatar"
          class="user-avatar"
        />
        <span class="user-name">Shirdon H.</span>
        <!-- Optional: Dropdown for user settings, logout, etc. -->
        <!-- <i class="fas fa-chevron-down"></i> -->
      </div>
    </div>
  </nav>
</template>

<script>
export default {
  name: 'TopNav',
  methods: {
    toggleSidebar() {
      // Emit an event or use a store (like Vuex/Pinia) to toggle the sidebar
      // This is a common pattern for mobile responsiveness
      console.log('Toggle sidebar clicked')
      // this.$emit('toggle-sidebar');
    },
  },
}
</script>

<style scoped>
.top-navigation {
  display: flex;
  justify-content: space-between;
  align-items: center;
  width: 100%;
  height: 100%; /* Takes full height of its container from dashboardPage.vue */
  padding: 0 20px; /* Matches padding in dashboardPage.vue's .top-nav-container */
  background-color: var(
    --dashboard-secondary-bg,
    #ffffff
  ); /* Use variable from parent or default */
  color: var(--dashboard-text-color, #333333);
  box-sizing: border-box;
}

.nav-left,
.nav-center,
.nav-right {
  display: flex;
  align-items: center;
}

.brand-logo {
  font-size: 1.5rem;
  font-weight: bold;
  color: var(--dashboard-sidebar-bg, #2c3e50); /* Example: Use sidebar bg for contrast */
  margin-right: 20px;
}

.nav-right .nav-item {
  margin-left: 20px;
  display: flex;
  align-items: center;
  cursor: pointer;
}

.notifications {
  position: relative;
}

.notification-badge {
  position: absolute;
  top: -5px;
  right: -8px;
  background-color: #e74c3c; /* Red for notifications */
  color: white;
  border-radius: 50%;
  padding: 2px 6px;
  font-size: 0.7rem;
  font-weight: bold;
}

.user-avatar {
  width: 36px;
  height: 36px;
  border-radius: 50%;
  margin-right: 10px;
  object-fit: cover;
  border: 2px solid var(--dashboard-border-color, #e0e0e0);
}

.user-name {
  font-weight: 500;
}

/* Add styles for .mobile-menu-toggle, .search-bar, icons (e.g., Font Awesome) as needed */
/* For icons, you'd typically install an icon library like Font Awesome or use SVGs */
/* Example for Font Awesome (ensure it's installed and imported) */
/* @import '~@fortawesome/fontawesome-free/css/all.css'; */

.mobile-menu-toggle {
  display: none; /* Hidden by default, shown on mobile */
  background: none;
  border: none;
  font-size: 1.5rem;
  cursor: pointer;
  color: var(--dashboard-text-color, #333333);
}

@media (max-width: 768px) {
  .user-name {
    display: none; /* Hide username on smaller screens, show only avatar */
  }
  .brand-logo {
    font-size: 1.2rem;
  }
  /* Example: Show hamburger menu
  .mobile-menu-toggle {
    display: block;
    margin-right: 15px;
  }
  */
}
</style>
